...
internal static unsafe class Program
{
    ...
    private static FontManager fs = null;
    ...
    {
        fs.GetAtlasSize(out var w, out var h);
        if (w < h)
            w *= 2;
        else
            h *= 2;
    ...
    private static void StashError(FonsErrorCode error, int val)
    {
        switch (error)
        {
            ...
            case FonsErrorCode.ScratchFull:
                Console.WriteLine("Scratch full, tried to allocate " + val + " has " + FontManager.ScratchBufSize);
                break;
    ...
    static void Main(string[] args)
    {
        ...
        var fontNormal = FontManager.Invalid;
        var fontItalic = FontManager.Invalid;
        var fontBold = FontManager.Invalid;
        WindowHandle* window;
        VideoMode* mode;
        ...
        window = glfw.CreateWindow(mode->Width - 40, mode->Height - 80, "FontStash.NET - Error", null, null);
        if (window == null)
        {
            glfw.Terminate();
            Environment.Exit(-1);
        ...
        GlFontLegacy glfons = new(gl);
        fs = glfons.Create(512, 512, FontFlags.ZeroTopLeft);
        ...
        while (!glfw.WindowShouldClose(window))
        {
            ...
            var white = GlFontLegacy.Rgba(255, 255, 255, 255);
            var brown = GlFontLegacy.Rgba(192, 128, 0, 128);
            var blue = GlFontLegacy.Rgba(0, 192, 255, 255);
            var black = GlFontLegacy.Rgba(0, 0, 0, 255);
            ...
            fs.GetAtlasSize(out var atlasw, out var atlash);
            var msg = "Atlas: " + atlasw + " X " + atlash;
